...
app.controller('gridmapctl',
    ['$scope', '$rootScope', '$http', '$compile', '$state','$timeout', 'localStorageService', 'userService', 'gridmapService',
        function ($scope, $rootScope, $http, $compile, $state, $timeout,localStorageService, userService, gridmapService) {
            $rootScope.mapEngine = {};
            //一个地图上的所有可移动目标
            ...
            //控制显示摄像头位置
            $rootScope.showcamera_position = function (e) {
                if (e) {
                    $rootScope.mapEngine.engineConfig.showcameraposition = true;
                } else {
                    $rootScope.mapEngine.engineConfig.showcameraposition = false;
                }
                console.log($rootScope.mapEngine.engineConfig.showcameraposition);
            }
            /*
             * 刷新地图上所有活动点
             */
                    ...
                    for (var indd = 0; indd < $rootScope.movingObjs.length; indd++) {
                        if ($rootScope.movingObjs[indd]._id == id)//不管是什么物体，唯一id就可以标示
                        {
                            if ($rootScope.movingObjs[indd].mark) {
                                ...
                                $rootScope.movingObjs[indd].mark.setPosition(position)
                            } else {
                                $rootScope.currentperson++;
                                console.log('构造新的mark')
                                //构造新的mark
                            ...
                            if ($rootScope.movingObjs[indd].infoWin) {
                                var openeda = $rootScope.movingObjs[indd].infoWin.getIsOpen();
                                if (openeda) {
                                ...
                                AMap.event.addListener($rootScope.movingObjs[indd].mark, 'click', function () {
                                    ...
                                    if (!opened) {
                                        $rootScope.movingObjs[indd].infoWin.open($scope.map, position);
                                    } else {
...
//console.log(infowindow3)
            ...
            /*
             加载案件位置
             */
            $scope.geteventposition = function () {
                var dep = [];
                for (d = 0; d < $rootScope.curUser.departments.length; d++) {
                    dep.push($rootScope.curUser.departments[d].department)
                }
                gridmapService.getdarpmenteventposition(dep,$rootScope.curUser._id,$rootScope.applicationServerpath)
            }
            //绘制案件地图位置
            $scope.draweventposition=function(id,position,name,time,info){
                //console.log(position, name, time, info)
                var newMark;
                for(var index=0;index<$rootScope.eventpositionObjs.length;index++) {
                    if ($rootScope.eventpositionObjs[index]._id == id) {
                        if ($rootScope.eventpositionObjs[index].mark) {

                        } else {
                            //console.log('构造新的mark')
                            newMark = new AMap.Marker({
                                content: '<div ><img src="./img/book197.png" style="height:30px;width:30px"><br></div>',
                                position:position
                            });
                            $rootScope.eventpositionObjs[index].mark = newMark;
                            $rootScope.eventpositionObjs[index].mark.setMap($scope.map)
                            newMark.setTitle(name);
                            var label = {//label默认蓝框白底左上角显示，样式className为：amap-marker-label
                                offset: new AMap.Pixel(-10, 42),//修改label相对于maker的位置
                                content: name,
                                style: "{width:auto;    margin-left:1px;    float:left;    font-family:Arial, Helvetica, sans-serif;    font-size:13px;    color:#5f5f5f;    line-height:35px;    text-transform:uppercase}"
            ...
            //解析地图位置
            $scope.assembleeventMapInfo=function(){
                for(var index=0;index<$rootScope.eventpositionObjs.length;index++){
                    $rootScope.eventpositionObjs[index].newer=new Date($rootScope.eventpositionObjs[index].newer).formate('yyyy-MM-dd hh:mm:ss');
                    /******************************************************************************/
                    //构建信息窗体中显示的内容class="list "
                    var btntext ='<a ng-click=\'' + 'gotoChat("receiver._id")\'' + ' style="color:#00f" >点击可查看事件</a>';
                    var html = '<div ' +
